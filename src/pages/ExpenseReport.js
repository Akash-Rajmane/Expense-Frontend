...
const ExpenseReport = () => {
  ...
  const fetchExpenseData = async (selectedCategory) => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_BE_HOST}/premium/get-expenses-by-interval/${selectedCategory}`,
      {
        headers: {
  ...
  const fetchDownloadedFilesData = async () => {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_BE_HOST}/premium/get-downloaded-files-data`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const downloadHandler = async () => {
    ...
    return;
    const response = await fetch(
      `${process.env.REACT_APP_BE_HOST}/premium/download-expenses`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        ...
        <div className="premiumFeatures">
          {isLoadingInterval ? (
            <Loader
              ...
              height={40}
              width={40}
              className={"intervalReport"}
            />
          ) : (
            <div className="intervalReport">
              <Dropdown
