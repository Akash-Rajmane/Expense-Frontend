...
const ResetPassword = () => {
  ...
  const fetchPage = async (id) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BE_HOST}/password/reset-password/${id}`,
      );
  ...
  useEffect(() => {
    const handlePasswordReset = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.REACT_APP_BE_HOST}/password/update-password/${id}`,
          {
            method: "POST",
            headers: {
  ...
  return <div className="auth">{parse(htmlContent)}</div>;
};
